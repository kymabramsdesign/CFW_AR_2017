...
/*-----------------------------------------------------------------------------------*/
    ...
    function px_comment_fields($fields) {
    ...
    add_filter('comment_form_default_fields','px_comment_fields');

    function px_comment_form_before()
    {
        echo '<div class="form-fields clearfix">';
    }

    function px_comment_form_after()
    {
        echo '</div>';
    }

    add_action('comment_form_before_fields', 'px_comment_form_before');
    add_action('comment_form_after_fields', 'px_comment_form_after');

    //Comment styling

    function px_theme_comment($comment, $args, $depth) {
    ...
    function px_theme_list_pings($comment, $args, $depth) {
        $GLOBALS['comment'] = $comment; ?>
...
if ( have_comments() ) { // if there are comments ?>
    <div class="comments-wrap">
        ...
        <?php if ( ! empty($comments_by_type['comment']) ) { // if there are normal comments ?>
            ...
            <?php wp_list_comments('type=comment&avatar_size=64&callback=px_theme_comment'); ?>
            </ul>
        ...
        <?php if ( ! empty($comments_by_type['pings']) ) { // if there are pings ?>
            ...
            <ol class="ping_list">
            <?php wp_list_comments('type=pings&callback=px_theme_list_pings'); ?>
            </ol>
