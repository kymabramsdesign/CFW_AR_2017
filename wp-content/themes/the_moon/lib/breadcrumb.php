...
function px_get_post_parent_trail($post_id)
{
    $parents = array();
...
function px_modify_breadcrumb_single($postType, $trail, $delimiter)
{
    ...
    if(isset($_GET['pnt']))
    {
        ...
        {
            $parents = px_get_post_parent_trail($parent);
    ...
    else
    {
        ...
        $pages = px_search_pages_by_content('[portfolio');
        $page  = null;
        ...
        if($page)
        {
            $parents = px_get_post_parent_trail($page->ID);
