...
$template = new PxTemplate(px_path_combine(dirname(__FILE__), 'templates'));
...
<script class="px-sc-flags" type="text/html"><?php echo px_array_value('flags', $sc); ?></script>
<?php
...
foreach($sc['fields'] as $key => $settings)
{
    ...
    <div class="px-sc-section px-clearfix">
        ...
        echo $template->PX_GetTemplate($fname, array('key'=>$key, 'settings'=>$settings));
        ?>
    </div>
    <?php
}