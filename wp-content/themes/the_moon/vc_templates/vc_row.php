...
$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'section '.get_row_css_class().$el_class, $this->settings['base']);
...
if ($row_type == 'row') {
	...
	$output .= '<div id='.$id.' class="background_cover ' . $css_class_type .' ' . $css_class.'"';
    $output .= " style='";
    ...
    $output .='<div class="container"><div class="wpb_row vc_row-fluid parallax_content">';
    ...
    //$css_class_type =  " fullWidth";
    if(!$_image[0]){
        $_image[0]=THEME_IMAGES_URI."/placeholders/parallax-holder.jpg";
    }
    $output .='<div id='.$id.' class="parallax sectionOverlay '. $el_class .' '. $overlay_texture .'"  data-speed="'. $parallax_speed .'"  data-position="50" style = "';
    $output .= ($min_height !='' || $min_height!=' ') ? 'min-height:' . $min_height . 'px;' : '';
    ...
    $output .= '<div id='.$id.' class="videoHome sectionOverlay '. $css_class_type .' '. $overlay_texture .'" style="'.$rowspace.'">';	
    ...
    $output .='<div class="container"><div class="wpb_row vc_row-fluid videoContent vc_videocontent">';
